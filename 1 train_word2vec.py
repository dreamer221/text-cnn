...
def train_word2vec(filenames):
    ...
    argv: filenames: a list of train_filename, test_filename, val_filename
    return: save word vector to config.vector_word_filename
    '''
    sentences = Get_Sentences(filenames)
    print("类型:", type(sentences))
    logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO)

    model = word2vec.Word2Vec(sentences, size=100, window=5, min_count=1, workers=6)  # 默认epoch 是 5
