...
class TextConfig():
    ...
    seq_length = 300  # 600 max length of sentence
...
class TextCNN(object):
    ...
    def cnn(self):
        ...
        with tf.name_scope('output'):
            ...
            self.y_pred_cls = tf.argmax(self.logits, 1, name='predictions')  # 每一行最大值所在的列的索引
        ...
        with tf.name_scope('accuracy'):
            ...
            self.acc = tf.reduce_mean(tf.cast(correct_pred, tf.float32))  # 转换成float后求平均值
